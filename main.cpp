...
int main()
{
	...
	*/
	...
	{ //TOTAL:7699 .. 2179
		...
		if (n == 13)
		{											// to check if the data satisfy our required format
	...
	for (unsigned i = 0; i < 67663; ++i)
	{
		...
		{										// to check if the data satisfy our required format
			...
			//cout << i << endl;
			if (source_airport_airport == NULL || destination_airport_airport == NULL)
				continue;
	...
	*/
	...
	// cout << dijkstra.shortest_distance(airports[d]) << endl;

	if (dijkstra.shortest_distance(airports[d]) != numeric_limits<double>::infinity())
	{
